-- Gorge-droppable tunnel ends.
--
-- Ported from ENSL CompMod (lua/CompMod/Weapons/Alien/TunnelAbility.lua). The placement geometry
-- below is CompMod's and is kept verbatim; the B2TP addition is the room-power gate applied in
-- GetIsPositionValid. See Shared/PlacementRules.lua.

Script.Load("lua/Weapons/Alien/StructureAbility.lua")
Script.Load("lua/B2TP/GorgeTunnels/Shared/PlacementRules.lua")
...
function TunnelAbility:GetIsPositionValid(position, player, surfaceNormal)
    ...
    local valid, newCoords = CalculateTunnelPosition(position, player, surfaceNormal)

    -- B2TP: gorges may only drop into a room whose power node is down, or onto infestation.
    -- Evaluated here so the client ghost turns red and the server rejects from the same predicate;
    -- DropStructureAbility:DropStructure re-runs this server-side before charging the player.
    if valid and not B2TP_GetIsGorgeTunnelPositionAllowed(position) then
        valid = false
    end
