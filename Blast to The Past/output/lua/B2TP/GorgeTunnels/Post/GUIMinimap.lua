-- Minimap name tags for tunnel entrances ("Entry 2", "Exit 3").
-- Ported from ENSL CompMod (lua/CompMod/GUI/GUIMinimap.lua).
--
-- Uses its own pooled set of text items rather than vanilla's player name tag pool, so tunnel
-- labels and player labels cannot evict each other. The tag text comes from
-- AlienTunnelManager:GetTunnelNameTag, added by this piece's AlienTunnelManager hook.
