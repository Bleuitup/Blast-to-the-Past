-- Three-mode gorge build menu: structures -> tunnel network -> entrance/exit.
-- Ported from ENSL CompMod (lua/CompMod/GUI/GUIGorgeBuildMenu.lua).
--
-- Icons reuse existing rows of the vanilla gorge build atlas, so no new art is required.
...
function GUIGorgeBuildMenu:Initialize()
    ...
    -- 1 = structure menu, 2 = tunnel network selection, 3 = entrance/exit selection
    self.menuMode = 0
    self:SetMenuMode(1)
end
...
function GUIGorgeBuildMenu:CreateButton(techId, scale, frame, keybind, position)
    PROFILE("GUIGorgeBuildMenu:CreateButton")
    local button = oldCreateButton(self, techId, scale, frame, keybind, position)

    -- In vanilla, texture pixel coords are only set in the update function, which is fine when
    -- buttons are created in the constructor. Creating buttons on the fly can render a button
    -- before Update runs, flashing the entire icon grid for one frame. Setting the coordinates at
    -- creation time avoids that.
...
function GUIGorgeBuildMenu:DestroyButtons()
    ...
    for i = 1, #self.buttons do
        -- Deviation from CompMod, which destroys button.background here. GUIAnimatedItem's
        -- Cleanup only recurses into children, and button.frame is background's PARENT, so
        -- destroying background orphans the frame -- one leaked GUIItem per button, every time
        -- the menu changes mode. frame is the root of the whole button, so destroying it
        -- cascades to background and everything below it.
        self.buttons[i].frame:Destroy()
    end
