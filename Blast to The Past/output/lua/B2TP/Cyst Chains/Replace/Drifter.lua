...
local function FindTask(self)
    ...
    for _, structure in ipairs(GetEntitiesWithMixinForTeamWithinRange("Construct", self:GetTeamNumber(), self:GetOrigin(), kDrifterSelfOrderRange)) do
        ...
        if not structure:GetIsBuilt() and not IsBeingGrown(self, structure) and (not structure.GetCanAutoBuild or structure:GetCanAutoBuild()) then
            ...
            -- B2TP: automatic orders should not reconstruct full cyst chains.
...
function Drifter:OnOrderGiven(newOrder)
    ...
    -- B2TP: queue downstream cyst chain only for manual Grow orders on cysts.
    if self.reconstructingChain == nil
            and orderTarget
            and newOrder:GetType() == kTechId.Grow
            and orderTarget:isa("Cyst")
            and not self.isAutomaticOrder then
...
function Drifter:ReConstructCystChain(cysts, cystChain, currCyst, depth)
    ...
    -- Long cyst chains are finite; this prevents accidental infinite recursion.
    if depth > 15 then
        return
    end
